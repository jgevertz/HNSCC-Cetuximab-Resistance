...
% Numerically fits Model 3.2 with both pre-existing and drug-induced    %
% acquired resistanc to each treatment mouse (using censored data).     %
% Algorithm is run 15 times, and saves the best parameter set per mouse %
% from the 15 runs. Parallel toolbox required, with access to 29        %
% parallel pools.                                                       %
%                                                                       %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear all; close all;

path = 'Best_Fit_Model3_2';
if exist(path, 'dir') ~= 7
    mkdir(path)
end
    ...
    for rep = 1:numreps
        ...
        for u = 1:n_Sbpts % n_pts  % Solve DE at all biologically-relevant Sobol points
            if ((uniform_sobol_scaled(u,4) > uniform_sobol_scaled(u,5))&&(uniform_sobol_scaled(u,1)>=uniform_sobol_scaled(u,2)))
                ...
                sb_soln{d1} = ode23s(@(t,x) ExpDrugModel_Drug_causes_resistance(t,x,uniform_sobol_scaled(u,1),...
                ...
                for d = 2:dosage
                    sb_soln{d} = ode23s(@(t,x) ExpDrugModel_Drug_causes_resistance(t,x,uniform_sobol_scaled(u,1),...
                        uniform_sobol_scaled(u,2),uniform_sobol_scaled(u,3), uniform_sobol_scaled(u,4),...
        ...
        order_g = floor(log10(newParam_g(1)))-1;
        ...
        for j = 1:n_SApts
            ...
            d1 = 1;
            sa_soln = {};
            sa_soln{d1} = ode23s(@(t,x) ExpDrugModel_Drug_causes_resistance(t,x,tempnewparam_r_s,tempnewparam_r_r,...
                tempnewparam_g, tempnewparam_lambda_s,tempnewparam_lambda_r,gamma),0:7,...
                [tempnewy0,tempnewy0r,5]);
            t1 = 7;  t2 = 14;
            
            for d = 2:dosage
                sa_soln{d} = ode23s(@(t,x) ExpDrugModel_Drug_causes_resistance(t,x,tempnewparam_r_s,tempnewparam_r_r,...
        ...
        d1 = 1;
        final_soln = {}
        final_soln{d1} = ode23s(@(t,x) ExpDrugModel_Drug_causes_resistance(t,x,newParam_r_s(end),newParam_r_r(end),...
            newParam_g(end), newParam_lambda_s(end),newParam_lambda_r(end),gamma),...
            0:7,[y0(end),y0r(end),5]);
        t1 = 7; t2 = 14;
        for d = 2:dosage
            final_soln{d} = ode23s(@(t,x) ExpDrugModel_Drug_causes_resistance(t,x,newParam_r_s(end),newParam_r_r(end),...
                newParam_g(end), newParam_lambda_s(end),newParam_lambda_r(end),gamma),...
        ...
        title(['Model 1.2: Mouse ' num2str(m) ' rep ' num2str(rep)],'FontSize',18)
