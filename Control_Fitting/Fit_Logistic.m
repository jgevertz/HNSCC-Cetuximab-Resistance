...
% Numerically fits logistic curve to each control mouse (using censored %
...
Best_r = zeros(num_mice,numreps + 1); Best_c = zeros(num_mice,numreps + 1);
Best_y0 = zeros(num_mice,numreps + 1); Best_SSE = zeros(num_mice,numreps + 1);
...
parfor m = 1:num_mice
    ...
    for rep = 1:numreps
        ...
        min_y0 = 0; max_y0 = 2*Volume_loop(1);
        min_c = Volume_loop(1); max_c = 10*Volume_loop(1);
        ...
        uniform_sobol = sobolset(3,'Skip',n_skip,'Leap',n_leap)
        ...
        uniform_sobol_scaled(:,2) = (max_c-min_c)*uniform_sobol(:,2) + min_c;
        uniform_sobol_scaled(:,3) = (max_y0-min_y0)*uniform_sobol(:,3) + min_y0;
        SSE_QMC = [];
        ...
        for u = 1:n_Sbpts % n_pts  % Solve DE at all biologically-relevant Sobol points
            ...
            pass_check = -1;
            % need y0<c when r<0. No restrictions when r>0
            if((uniform_sobol_scaled(u,1)>0) || ...
                    ((uniform_sobol_scaled(u,1)<0)&&(uniform_sobol_scaled(u,3)<uniform_sobol_scaled(u,2)) ) )
                % need y0<c for logistic growth or decay to make sense
                                
                %create first soln vector
                sb_soln = ode23s(@(t,x) logistic(t,x,uniform_sobol_scaled(u,1),...
                    uniform_sobol_scaled(u,2)),...
                    Days_loop(1):Days_loop(end),uniform_sobol_scaled(u,3));
        ...
        newParam_r = []; newParam_c = [];
        y0 = []; SSE = []; SSEstep = [];
        
        i = 1;
        newParam_r(i) = uniform_sobol_scaled(index,1);
        newParam_c(i) = uniform_sobol_scaled(index,2);
        y0(i)=  uniform_sobol_scaled(index,3);
        SSE(i) = c;
        ...
        for j = 1:n_SApts
            ...
            s = ((b-a)*(rand)+a)*order_c;
            v = ((b-a)*(rand)+a)*10^(order_y0);
            
            tempnewparam_r = newParam_r(i) + r;
            tempnewparam_c = newParam_c(i) + s;
            tempnewy0 = y0(i) + v;
            
            % no negative K, c or y0, and cannot have y0>K when r<0
            while ( (tempnewparam_c<0) || (tempnewy0<0) || ((tempnewparam_r<0) && (tempnewy0>tempnewparam_c)) )
                r = ((b-a)*(rand)+a)*10^(order_r);
                s = ((b-a)*(rand)+a)*10^(order_c);
                v = ((b-a)*(rand)+a)*10^(order_y0);
                tempnewparam_r = newParam_r(i) + r;
                tempnewparam_c = newParam_c(i) + s;
                tempnewy0 = y0(i) + v;
            ...
            sa_soln = ode23s(@(t,x) logistic(t,x,tempnewparam_r,...
            ...
            if (newr <= probability)
                %{
                if mod(i,10) == 0
                    fprintf('\t%d. Mouse %d rep %d: New SSE = %f, old SSE = %f\n', i, m, rep, newz, SSE(i));
                    fprintf('\tr_s = %f, r_r = %f, lambda_s = %f, lambda_r = %f, y0 = %f, y0r = %f\n',...
                        tempnewparam_r_s, tempnewparam_r_r, tempnewparam_lambda_s, tempnewparam_lambda_r,...
                        tempnewy0, tempnewy0r);
                end
                %}
                i = i + 1;
                SSE(i) = newz;
        ...
        final_soln = ode23s(@(t,x) logistic(t,x,newParam_r(end),...
        ...
        title(['Logistic: Mouse ' num2str(m) 'rep ' num2str(rep)],'FontSize',18)
        
        fprintf('For Mouse %d rep %d, Best_r = %f, Best_c = %f\nBest_SSE = %f, Best_S0 = %f\n',...
            m, rep, Best_r(m,rep), Best_c(m,rep),...
...
for mouse2 = 1:num_mice
    ...
    %{
figure;
    plot(1:size(SA_SSEs1{mouse2,numreps + 1}),SA_SSEs1{mouse2,numreps + 1}(:));
    xlabel('SA step','FontSize',16);
    ylabel('SSE','FontSize',16);
    title(['Logistic: Mouse ' num2str(mouse2) 'rep ' num2str(minrep)],'FontSize',18)
...
Final_r = Best_r(:,numreps + 1)'; Final_c = Best_c(:,numreps + 1)';
Final_SSE = Best_SSE(:,numreps + 1)'; Final_y0 = Best_y0(:,numreps + 1)';


filename = [path '/BestParams.mat'];
save(filename,'Best_r','Best_c',...
    'Best_SSE','Best_y0','BestRep','Final_r',...
